...
to process all four rows. Here's another example, suppose we are doing an elemenwise add:
...
It's an optimzation for the reduce axis, so I will use the ast from our sum example, and apply the opt as `Opt(OptOps.GROUP, 0, 4)`,
resulting kernel is:
