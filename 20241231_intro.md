...
The second part is the compiler, it's main concern is how to generate GPU code for any given tensor computation and
handle the relevant scheduling (i.e. "when and how to run those generated code").

Let's use the simpliest computation example, where we are adding two tensors element-wise:
...
we launched 16 unit of computation. You may see "E_4_4", meaning we launched 4 units, and each unit further launched
four sub unit, totalling 16 computation (in CUDA or Metal parlance, we launched 4 blocks, and each block launched 4
threads). The details is of another topic.
...
GPU code, so a lot of complexity in learning lies in the "how", rather than explaining "what" these code are doing.
Think of it this way, suppose you are already hand writing a lot of C++ CUDA/Metal code by hand, tinygrad makes that easier
by asking you to only describe the high level operation in the form of tensor operations. Feel free to look up CUDA
...
optimization techniques. But nevertheless, let's see a few other examples to familiarize ourselves with the UOp.
...
Let me summarize what we have done so far. We looked at the tinygrad internals from the compiler perspective, mainly concerning
it's ability to render GPU code. This rendering process is handled by a "pattern matcher", that progressively turn the
...
In this process, I didn't yet cover how the conversion from tensor to high level UOp happens. This is referred to
as "scheduling", because each UOp represent a kernel, that's also where kernel fusion happens.

I also didn't yet cover how the "lower" process works (high level UOp -> low level UOp), but you can read the
[VIZ post](20241129_viz.md) for a taste of it.

One part we briefly touched upon is `NOOPT=1` flag, that converts the 16 grids op into 4 grid with vectorized data. This
is covered in my other posts on [kernel optimization & beam search](20241203_beam.md)

You should also read the [shapetracker post](20241217_st.md), together with [matmul](20241203_matmul.md) and 
[conv](20241208_conv.md), which covers a fundamental ideas in tinygrad, that almost everything can be represented
by "elementwise" + "reduce" + zero cost shape movement.

For a taste of GPU abstraction, check out the [Metal post](20240921_metal.md), it applies somewhat to WebGPU.